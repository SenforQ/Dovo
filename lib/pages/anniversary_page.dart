...
class _AnniversaryPageState extends State<AnniversaryPage> {
  List<Map<String, dynamic>> _anniversaryItems = [];
  ...
  Future<void> _loadData() async {
    ...
    await _loadAnniversaryItems();
  }

  Future<void> _loadAnniversaryItems() async {
    try {
      ...
      // Extract items with Anniversary tag from DovoShowPhotoArray
      List<Map<String, dynamic>> anniversaryItems = [];
        ...
        if (photoArray is List) {
          for (var photo in photoArray) {
            if (photo is Map) {
              ...
              if (tags is List && tags.contains('Anniversary')) {
                // Add photo data with figure data
                anniversaryItems.add({
                  'photoData': photo,
                  'figureData': figure,
                });
              }
      ...
      if (mounted) {
        setState(() {
          _anniversaryItems = anniversaryItems;
          _loading = false;
        });
      }
    } catch (e) {
      debugPrint('Error loading anniversary items: $e');
      if (mounted) {
        setState(() {
          _anniversaryItems = [];
          _loading = false;
          ...
          children: [
            ...
            Expanded(
              child: _loading
                  ...
                  : _anniversaryItems.isEmpty
                      ? const Center(child: Text('No anniversary items found'))
...
class _AnniversaryCardItem extends StatelessWidget {
