...
class _HolidayPageState extends State<HolidayPage> {
  List<Map<String, dynamic>> _holidayItems = [];
  ...
  Future<void> _loadData() async {
    ...
    await _loadHolidayItems();
  }

  Future<void> _loadHolidayItems() async {
    try {
      ...
      // Extract items with Holiday tag from DovoShowPhotoArray
      List<Map<String, dynamic>> holidayItems = [];
        ...
        if (photoArray is List) {
          for (var photo in photoArray) {
            if (photo is Map) {
              ...
              if (tags is List && tags.contains('Holiday')) {
      ...
      if (mounted) {
        setState(() {
          _holidayItems = holidayItems;
          _loading = false;
        });
      }
    } catch (e) {
      debugPrint('Error loading holiday items: $e');
      if (mounted) {
        setState(() {
          _holidayItems = [];
          _loading = false;
            ...
            Expanded(
              child: _loading
                  ...
                  : _holidayItems.isEmpty
                      ? const Center(child: Text('No holiday items found'))
...
class _HolidayCardItem extends StatelessWidget {
